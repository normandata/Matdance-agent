...
## v1.1.20-hot-fix-2 摘要

v1.1.20-hot-fix-2 延续前几个 preview 版本的边界和长期运行可靠性工作。
...
- 已修复：技能整理可能因模型上下文超限，或在同一段异常证据上反复失败，从而阻塞后续时间线继续发现新技能。现在技能整理会对批次大小和 `skill_read` 窗口做可回升的降级，按轮次判定已读技能是否相关，只注入 tool call 而不注入原始 tool result，并在同一证据段反复可恢复失败后 skip 该毒性批次。
