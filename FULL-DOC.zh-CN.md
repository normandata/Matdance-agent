...
v1.1.21-preview 重点优化长会话里的上下文压缩，同时保留前几个 preview 版本已经完成的可靠性修复。

- README 被拆成简洁入口、专题文档和这份完整文档。README 不再承担所有解释。
- 主会话上下文压缩现在按完整请求预算估算，包括 system/context 消息、会话消息和 tool schema 开销。需要压缩时，系统把摘要作为 session state 里的 sidecar 保存，不改写原始会话记录。
- 自动压缩会保护最近 3 轮 user 及其后续 assistant/tool 消息，对更早历史做按 token 均衡的分段压缩；过大的分段会继续拆分，必要时退化成本地摘要，后续成功轮次可回升到更大的分段。
- 压缩重试会带一个由主 agent 写给下一步的 one-shot handoff。这个 handoff 只在压缩后的那次请求里注入一次，模型调用结束后移除，避免后续回合反复看到陈旧交接信息。
- 手动 `/compact` 现在和自动请求预算压缩使用同一套 compressor 路径，并且摘要会被保留下来供后续回合继续使用。
- 如果上游模型仍返回 context limit 错误，Matdance 会根据本次请求估算反推并更新配置里的 context window，保存调整后的值，强制压缩后重试一次。
- 系统内置记忆整理和技能整理任务改用英文稳定注册文案，Web UI 按当前语言显示系统任务标题和说明。
- 后台事件和子任务阶段文本统一改为英文运行数据，避免英文界面夹杂中文状态。
- 闲时技能验证曾因 `needs_changes` 技能反复自动重试而形成后台请求风暴；现在改为全局低频去重队列，默认 6 小时一轮、串行验证、每轮最多 3 个，且当前 fingerprint 下的 `needs_changes` / `invalid` 不再自动重试。
- 已在 `v1.1.20-hot-fix-2` 修复：技能整理可能因模型上下文超限，或在同一段异常证据上反复失败，从而阻塞后续时间线继续发现新技能。现在技能整理会对批次大小和 `skill_read` 窗口做可回升的降级，按轮次判定已读技能是否相关，只注入 tool call 而不注入原始 tool result，并在同一证据段反复可恢复失败后 skip 该毒性批次。
...
早期闲时验证曾把验证报告状态为 `needs_changes` 或 `invalid` 的技能继续送回自动验证队列。这个设计初衷是避免技能停在半修复状态，但它在真实后台运行里变成了 P0 风险：当 skill 内容指纹没有变化、报告仍然是当前报告时，worker 仍会把同一批 `needs_changes` 技能按分钟重复验证；如果上游返回 429、quota 或 rate limit，普通模型重试链还会继续探测，导致用户睡眠或离开期间产生大量后台请求。

v1.1.20-preview 后，自动技能验证改成全局低频去重队列。队列项按 `agent + skillId + fingerprint` 去重，默认每 6 小时出队一轮，每轮串行验证 1 个，Settings 最多允许 3 个。只有缺少当前验证报告、验证报告过期或 skill 内容变化时，技能才会进入自动队列；当前 fingerprint 下的 `needs_changes` / `invalid` 不再自动重试。自动验证遇到 429、quota、rate limit 或 high demand 会停止本轮并退避到下一次间隔。手动验证仍然即时可用；如果 agent 在实际业务中发现技能不符、步骤过期或有明确改进，应直接用 `skill_editor` 修复，而不是依赖后台循环反复消耗请求。
...
技能验证不参与启动补偿。它不是时间债务，而是状态债务：只有当某个 skill 缺少当前验证报告、验证报告过期，或 skill 内容变化导致 fingerprint 失效，并且 agent 处于空闲状态时，idle validation worker 才会把它放入全局自动验证队列。自动队列按 `agent + skillId + fingerprint` 去重，按 Settings 配置的低频窗口出队，默认每 6 小时串行验证 1 个，最多 3 个。报告为 `needs_changes` 或 `invalid` 但 fingerprint 未变化时，不再自动入队；这类问题应由手动验证、明确修复或实际使用中触发的 `skill_editor` 修复推进。
...
技能整理不会把所有技能全文塞进上下文。它先读取技能索引，再按关联性调用 `skill_read` 定向阅读。已在 `v1.1.20-hot-fix-2` 修复：`skill_read` 现在按轮次工作，每轮只有一个较小且可降级的窗口；每轮之后 subagent 必须立刻判定每个已读技能是相关还是不相关。不相关说明书会在下一轮前丢弃；只有被保留为相关的技能，才允许被 update、delete 或 supersede。源会话里的 tool result 原文不会注入技能整理上下文，只保留 tool call 名称和参数作为可复用流程证据。

技能整理现在采用类似记忆整理的“向下失败、向上恢复”策略。遇到 context、payload、token 或结构化输出相关失败时，会缩小待处理消息批次或 `skill_read` 窗口；后续批次成功后再逐步回升到默认值。如果同一段证据在降级后仍然失败超过两次，系统会把它作为毒性批次 skip，让后续时间线仍能继续创建或更新技能。
