...
v1.1.18-preview 是在 v1.1.17 的长期运行可靠性基础上继续补边界的一版。
...
- `browser_evaluate`、导航、截图、正文读取、标题读取和浏览器全局操作锁都有超时边界，避免一个工具调用无限卡住队列。
...
- 导航、截图、正文读取、标题读取和全局操作锁都有超时边界，避免长期占住浏览器队列。
...
`image_generation` 通过 Settings 里配置的 OpenAI-compatible `/images/generations` 端点生成图片，并把结果保存到当前 agent 的 `workspace/generated/images/`。不传 `profile` 时，会按当前配置里的默认/自动 profile 顺序尝试启用的图像模型。
