...
- `scheduled_task_do` is a manual test run for user-created tasks and should only be used when the user asks for it. It does real work and delivers the result to the configured targets by default, but it does not advance `nextRunAt`, `runCount`, failure count, backoff, or the last scheduled-run fields. System built-in tasks cannot be manually tested.
