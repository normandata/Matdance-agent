...
- `created_session`：创建任务时所在的普通聊天会话。
- `session`：用户用 `session_list` 确认过的某一个普通聊天会话 ID。
- `all_agent_sessions`：当前 agent 自己的全部普通会话。
- `notification_session`：为该任务创建一个专属只读通知会话。用户看到的会话名是任务 title，后台仍然使用唯一 session id 路由和持久化。

专属通知会话会出现在会话列表里，也会持久化，但聊天输入区会被禁用。它只接收定时任务结果投递；当前阶段，记忆整理和技能整理都会忽略这种会话，全部会话投递也会跳过它。

投递后的定时任务通知卡片会完整渲染 subagent 的 Markdown 输出。前端只剥离 Matdance 固定的低优先级脚注；subagent 结果正文里的分隔线、表格、标题和后续内容都会保留。
...
- `session_list`：列出当前 agent 的会话 ID、展示标题、类型/只读标记和短元信息，用于让用户确认普通投递目标会话。
- `scheduled_task_create`：只有在时区、定时规则、任务内容和投递目标都明确后才创建任务。默认投递目标是发起任务的当前普通会话；“全部会话”只表示当前 agent 自己的全部普通会话；指定历史会话必须先让用户确认准确的 `sessionId`；新建专属通知会话则使用 `notification_session`。
...
- `scheduled_task_do_a_test`：仅在用户要求测试用户级任务时排队运行一次。主 agent 工具调用不会同步执行测试，而是送入最高优先级定时任务队列，等当前会话回合释放并发预算后再启动。测试会真实办事并默认投递到配置目标，但不会推进 `nextRunAt`、`runCount`、失败次数、退避或“上次规则运行”字段。系统内置任务不可手动测试。
