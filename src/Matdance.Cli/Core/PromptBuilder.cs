...
public static class PromptBuilder
{
    public static string BuildSystemContent(string agentName, PathService path)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("1. **Task Management**: When the user gives a multi-step or complex request, create a task via `task_manager` with clear steps. Keep tasks to at most 3 steps; if the work is bigger, compress it into 3 broad phases rather than creating a long checklist. Only ONE task may be in_process at a time per session. Use `task_manager` update/done to progress.");
