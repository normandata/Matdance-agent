...
Earlier idle validation allowed skills with `needs_changes` or `invalid` reports to keep entering the automatic validation queue. The intent was to keep half-repaired skills moving, but real background operation turned that into a P0 risk: when a skill fingerprint had not changed and the validation report was still current, the worker could still validate the same `needs_changes` skills every minute after the idle threshold. If the provider returned 429, quota, or rate-limit errors, the normal model retry chain kept probing, producing a large number of background requests while the user was asleep or away.

After v1.1.20-preview, automatic skill validation uses a global low-frequency deduplicated queue. Queue items are keyed by `agent + skillId + fingerprint`; the default interval is 6 hours; each window validates serially, default 1 and configurable up to 3. A skill enters the automatic queue only when the current validation report is missing, stale, or invalidated by changed skill content. Current-fingerprint `needs_changes` / `invalid` reports no longer auto-retry. If automatic validation hits 429, quota, rate limit, or high demand, it stops the current window and backs off until the next interval. Manual validation remains immediate. If real task use proves that a skill does not fit the business case, has stale steps, or has a clear improvement, agents should repair it directly with `skill_editor` instead of relying on background loops.
...
Skill validation is not part of startup compensation. It is state debt, not time debt: only when a skill is missing a current validation report, has a stale report, or has changed content that invalidates the fingerprint, and when the agent is idle, does the idle validation worker put it into the global automatic validation queue. The automatic queue is deduplicated by `agent + skillId + fingerprint` and drains in low-frequency windows configured in Settings, defaulting to one serial validation every 6 hours and capped at 3 per window. A current-fingerprint `needs_changes` or `invalid` report does not automatically re-enter the queue; those issues should move through manual validation, explicit repair, or evidence-based `skill_editor` updates during real use.
