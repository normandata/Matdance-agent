...
v1.1.17 的重点是长期运行可靠性和状态迁移：Web UI 托管进程会从 shadow 目录启动，源码入口在需要 build/run 前会尽量暂停并恢复占用服务；后台任务按 agent 独立并发预算调度，并通过 active run、heartbeat、stalled 退避、手动重试/修复重试避免旧任务永久卡住队列。浏览器 cookie 工具、提示音 zip 导入导出、用户时区对齐和远程 Web token 鉴权也都在这一版里整理进系统链路。
...
Web UI 默认应该只绑定本机地址。`localhost`、`127.0.0.1` 和 `::1` 会直接允许；如果你把 `--host` 改成 `0.0.0.0`、局域网 IP 或其他非 loopback 地址，后端会拒绝启动，除非你显式设置 `MATDANCE_ALLOW_REMOTE_WEB=1`。远程绑定会启用单 token 鉴权：同一时间最多只有一个 token 生效，浏览器登录成功后写入 HttpOnly cookie，系统 API 可以用 `Authorization: Bearer <token>` 或 `X-Matdance-Token`。你可以提前设置 `MATDANCE_WEB_TOKEN`；如果没设置，服务首次远程启动时会生成 token，打印到控制台，并保存到 `.matdance/state/web-auth.json`。
...
agent 不确定有哪些图像提供商时，应该先调用 `image_generation_list_profiles`。如果用户没有指定提供商，通常直接调用 `image_generation` 并省略 `profile`，让系统走默认/自动 profile 顺序；如果用户明确说了 profile 名称，再把它传给 `profile`。
...
- `image_generation`：调用 `/images/generations`，生成文件默认保存到 `agents/<agent>/workspace/generated/images/`。
